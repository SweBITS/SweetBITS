"""
tests.test_validation
Tests for Parquet metadata validation logic.
"""

import pytest
import polars as pl
from pathlib import Path
from sweetbits.metadata import validate_sweetbits_file, save_companion_metadata, get_standard_metadata
from sweetbits.tables import generate_table_logic

def test_validate_non_sweetbits_parquet(tmp_path):
    """Tests that a standard Parquet file (no metadata companion) fails validation."""
    pfile = tmp_path / "plain.parquet"
    df = pl.DataFrame({"sample_id": ["S1"], "t_id": [1], "clade_reads": [10], "taxon_reads": [10]})
    df.write_parquet(pfile)
    
    with pytest.raises(FileNotFoundError, match="Missing companion metadata file"):
        validate_sweetbits_file(pfile, expected_type="REPORT_PARQUET")

def test_validate_invalid_json_companion(tmp_path):
    """Tests that a companion file without a version fails."""
    pfile = tmp_path / "plain.parquet"
    df = pl.DataFrame({"sample_id": ["S1"], "t_id": [1], "clade_reads": [10], "taxon_reads": [10]})
    df.write_parquet(pfile)
    import json
    with open(pfile.with_name(f"{pfile.name}.json"), "w") as f:
        json.dump({"random_key": "value"}, f)
        
    with pytest.raises(ValueError, match="lacks provenance"):
        validate_sweetbits_file(pfile, expected_type="REPORT_PARQUET")

def test_validate_wrong_type(tmp_path):
    """Tests that a SweetBITS file with the wrong type fails validation."""
    pfile = tmp_path / "wrong_type.parquet"
    df = pl.DataFrame({"sample_id": ["S1"], "t_id": [1], "clade_reads": [10], "taxon_reads": [10]})
    meta = get_standard_metadata(file_type="KRAKEN_PARQUET")
    df.write_parquet(pfile)
    save_companion_metadata(pfile, meta)
    
    with pytest.raises(ValueError, match="File type mismatch"):
        validate_sweetbits_file(pfile, expected_type="REPORT_PARQUET")

def test_validate_missing_columns(tmp_path):
    """Tests that a SweetBITS file missing required columns fails validation."""
    pfile = tmp_path / "missing_cols.parquet"
    # Missing 'taxon_reads'
    df = pl.DataFrame({"sample_id": ["S1"], "t_id": [1], "clade_reads": [10]})
    meta = get_standard_metadata(file_type="REPORT_PARQUET")
    df.write_parquet(pfile)
    save_companion_metadata(pfile, meta)
    
    with pytest.raises(ValueError, match="missing required columns: taxon_reads"):
        validate_sweetbits_file(pfile, expected_type="REPORT_PARQUET", required_columns=["sample_id", "taxon_reads"])

def test_table_validation_integration(tmp_path):
    """Tests that generate_table_logic raises a clear error on invalid input."""
    pfile = tmp_path / "invalid.parquet"
    df = pl.DataFrame({"a": [1]})
    df.write_parquet(pfile)
    
    output_csv = tmp_path / "out.csv"
    
    # generate_table_logic should catch this via validate_sweetbits_file
    with pytest.raises(FileNotFoundError, match="Missing companion metadata file"):
        generate_table_logic(pfile, output_csv)

def test_validate_old_version(tmp_path):
    """Tests that a file older than the minimum compatible version fails validation."""
    pfile = tmp_path / "old.parquet"
    df = pl.DataFrame({"sample_id": ["S1"], "t_id": [1], "clade_reads": [10], "taxon_reads": [10]})
    # Mocking metadata with an old version
    meta = get_standard_metadata(file_type="REPORT_PARQUET")
    meta["sweetbits_version"] = "0.0.1" 
    df.write_parquet(pfile)
    save_companion_metadata(pfile, meta)
    
    with pytest.raises(ValueError, match="older than the minimum compatible version"):
        validate_sweetbits_file(pfile, expected_type="REPORT_PARQUET")

def test_validate_future_version(tmp_path):
    """Tests that a file from a future version fails validation."""
    pfile = tmp_path / "future.parquet"
    df = pl.DataFrame({"sample_id": ["S1"], "t_id": [1], "clade_reads": [10], "taxon_reads": [10]})
    meta = get_standard_metadata(file_type="REPORT_PARQUET")
    meta["sweetbits_version"] = "99.0.0" 
    df.write_parquet(pfile)
    save_companion_metadata(pfile, meta)
    
    with pytest.raises(ValueError, match="generated by a newer version of SweetBITS"):
        validate_sweetbits_file(pfile, expected_type="REPORT_PARQUET")

...
def main():
    ...
    for sample in SAMPLES:
        ...
        with open(input_dir / f"{sample['id']}.kraken", "w") as f:
            for i, r in enumerate(s_reads):
                if r["status"] == "U": f.write(f"U\tR{i}\t0\t150|150\t0\t0:35\n")
                else: f.write(f"C\tR{i}\t{r['t_id']}\t{r['length']}\t{r['mhg']}\t{r['kmer_str']}\n")
        
        counts = {}
        for r in s_reads:
            if r["status"] == "C": counts[r["t_id"]] = counts.get(r["t_id"], 0) + 1
        
        report_clade = {}
        for tid, count in counts.items():
            lin = tree.get_lineage(tid)
            for node in lin: report_clade[node] = report_clade.get(node, 0) + count
    ...
    for r in all_reads:
        ...
        tid = r["t_id"]
        direct_counts[(tid, p)] = direct_counts.get((tid, p), 0) + 1
        for node in tree.get_lineage(tid):
            clade_sums_truth[(node, p)] = clade_sums_truth.get((node, p), 0) + 1

    def write_csv(name, data_dict, filter_empty_cols=True):
        if not data_dict: return
        nodes_to_keep = [tid for tid in data_dict if any(clade_sums_truth.get((tid, p), 0) > 0 for p in periods)]
        active_periods = [p for p in periods if any(clade_sums_truth.get((tid, p), 0) > 0 for tid in nodes_to_keep)]
        if not active_periods: active_periods = [periods[0]]
        with open(truth_dir / name, "w", newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["t_id"] + active_periods)
            for tid in sorted(nodes_to_keep):
    ...
    taxon_raw = {tid: {p: direct_counts.get((tid, p), 0) for p in periods} for tid in all_present_nodes}
    write_csv("abundance_taxon.csv", taxon_raw, filter_empty_cols=False)
    
    clade_raw = {tid: {p: clade_sums_truth.get((tid, p), 0) for p in periods} for tid in all_present_nodes}
    ...
    for r in all_reads:
        ...
        p = f"{r['year']}_{r['week']:02d}"
        target = None
        for node in reversed(tree.get_lineage(r["t_id"])):
    ...
    bacteria_raw = {tid: clade_raw[tid] for tid in all_present_nodes if is_descendant(tid, 2)}
    active_periods_b = [p for p in periods if any(clade_sums_truth.get((tid, p), 0) > 0 for tid in bacteria_raw)]
    with open(truth_dir / "abundance_clade_filtered_bacteria.csv", "w", newline='') as f:
        writer = csv.writer(f); writer.writerow(["t_id"] + active_periods_b)
        for tid in sorted(bacteria_raw.keys()): writer.writerow([tid] + [bacteria_raw[tid].get(p, 0) for p in active_periods_b])
    ...
    def generate_filtered_truth(filter_type, threshold):
        ...
        for tid in leaf_to_root:
            passes = False
            if filter_type == "reads": passes = any(clade_raw[tid].get(p, 0) >= threshold for p in periods)
            elif filter_type == "obs": passes = sum(1 for p in periods if clade_raw[tid].get(p, 0) > 0) >= threshold
            if passes or tid == 1: final_kept[tid] = current_counts[tid]
            else:
                p_id = node_lookup[tid]['parent']
                if p_id in current_counts:
                    for p in periods: current_counts[p_id][p] += current_counts[tid][p]
        return final_kept
    ...
    with open(truth_dir / "kraken_inspect.csv", "w", newline='') as f:
        writer = csv.writer(f, delimiter=';'); writer.writerow(["tax_id", "clade_minimizers"])
        for tid, mm in db_minimizers.items(): writer.writerow([tid, mm])
    ...
    for s_tid, hits in species_map.items():
        ...
        res = {'t_id': s_tid, 'grand_clade_kmers': 0, 'grand_lineage_kmers': 0, 'grand_misclassified_kmers': 0, 'grand_root_kmers': 0, 'grand_total_kmers': sum(hits.values())}
        ...
        for k_tid, count in hits.items():
            ...
            if is_clade: res['grand_clade_kmers'] += count
                ...
                res['grand_exclade_kmers'] = res.get('grand_exclade_kmers', 0) + count; exc_hits[k_tid] = exc_hits.get(k_tid, 0) + count
                if is_lineage:
                    res['grand_lineage_kmers'] += count; lineage_dist_data.append({'val': k_meta['depth'] / max(int(s_meta['depth']) - 1, 1), 'weight': count})
                else:
                    res['grand_misclassified_kmers'] += count; misc_hits[k_tid] = misc_hits.get(k_tid, 0) + count
        ...
        c, d_class, d_tot, d_exc, d_misc = res, max(res['grand_classified_kmers'], 1), max(res['grand_total_kmers'], 1), max(res.get('grand_exclade_kmers', 0), 1), max(res['grand_misclassified_kmers'], 1)
        res.update({
            'grand_clade_to_classified_kmer_ratio': c['grand_clade_kmers'] / d_class, 'grand_lineage_to_classified_kmer_ratio': c['grand_lineage_kmers'] / d_class, 'grand_misclassified_to_classified_kmer_ratio': c['grand_misclassified_kmers'] / d_class, 'grand_root_to_classified_kmer_ratio': c['grand_root_kmers'] / d_class, 'grand_supporting_to_misclassified_kmer_ratio': (c['grand_clade_kmers'] + c['grand_lineage_kmers']) / d_misc if c['grand_misclassified_kmers'] > 0 else 1.0,
            'grand_clade_to_total_kmer_ratio': c['grand_clade_kmers'] / d_tot, 'grand_classified_to_total_kmer_ratio': c['grand_classified_kmers'] / d_tot, 'grand_lineage_to_total_kmer_ratio': c['grand_lineage_kmers'] / d_tot, 'grand_root_to_total_kmer_ratio': c['grand_root_kmers'] / d_tot, 'grand_misclassified_to_total_kmer_ratio': c['grand_misclassified_kmers'] / d_tot, 'grand_exclade_to_total_kmer_ratio': c.get('grand_exclade_kmers', 0) / d_tot, 'grand_supporting_to_total_kmer_ratio': (c['grand_clade_kmers'] + c['grand_lineage_kmers']) / d_tot,
            'grand_root_to_exclade_kmer_ratio': c['grand_root_kmers'] / d_exc, 'grand_lineage_to_exclade_kmer_ratio': c['grand_lineage_kmers'] / d_exc,
        })
        for m in ['distance', 'depth', 'relative_lca_depth']:
            v_col = 'kmer_depth' if m == 'depth' else m
            s = get_weighted_stats([{'v': x[v_col], 'w': x['weight']} for x in dist_data], 'v', 'w')
            res.update({f'mean_grand_misclassified_kmer_{m}': s['mean'], f'median_grand_misclassified_kmer_{m}': s['median'], f'cv_grand_misclassified_kmer_{m}': s['cv'], f'p05_grand_misclassified_kmer_{m}': s['p05'], f'p95_grand_misclassified_kmer_{m}': s['p95']})

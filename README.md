...
- `convert-kraken`: Ingests massive Kraken and FASTQ files into heavily compressed, memory-mapped `<KRAKEN_PARQUET>` data lakes using an extremely fast, multiprocessed two-pointer streaming engine. Supports creating "Skinny Parquets" (`--no-fastq`) that drop sequence payloads to save significant disk space while retaining all taxonomic intelligence.
